...
def show_class_prediction(file, models_path, temperature_path):
    div_scenarios = ["BD", "HE", "ME", "DD", "SR", "WW"]
    empirical_vec, empirical_resc_factor = load_trees_from_array([file], return_resc_factor=True)
    n_tips = str(len(empirical_vec[0]))

    classifier = load_model(models_path + 'old_class/' + n_tips + "_classification_model.keras") 
    
    temperature_classifier = load_model(models_path + 'old_class/' + n_tips + "_classification_temperaturemodel.keras")
    ...
    for i, pred in enumerate(prediction[0]):
        print(div_scenarios[i] + f': {pred:.4f}')
    ...
    predictions_MCMC = np.mean(preds, axis=0)
    
    print("----- Temperature model + MCMC Dropout prediction -----")   
    for i, pred in enumerate(predictions_MCMC[0]):
        print(div_scenarios[i] + f': {pred:.4f}')
...
def show_reg_prediction(file, div_scenario, models_path):
    empirical_vec, empirical_resc_factor = load_trees_from_array([file], return_resc_factor=True)
    n_tips = str(len(empirical_vec[0]))
    regressor = load_model(models_path + 'testing_k_reg/' + div_scenario + '/' + n_tips + "_regression_model.keras") 
    print(type(div_scenario))
    start_time = time()
    ...
    preds = []
    for _ in range(num_samples):
        p = regressor(empirical_vec, training=True) 
